...
import { Equipable } from "../typechain-types";
...
async function deployContracts(): Promise<void> {
    console.log(`Deploying Equipable to ${network.name} blockchain...`);

    const contractFactory = await ethers.getContractFactory("Equipable");
    const args = [
        "Equipable",
        "Equipable",
    ...
    const contract: Equipable = await contractFactory.deploy(...args);
    await contract.deployed();
    console.log(`Equipable deployed to ${contract.address}.`);
